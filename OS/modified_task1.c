...
void main (int argc, char *argv[]) 
{
	...
	// argv[1] the second, argv[2] the third, and so on and so forth.
        ...
        //Assuming that the second parameter, the program grabs that into an integer.
	int x = atoi(argv[1]);
	...
	//If process is the child process then pid==0, else it is the parent (the value contains the PID of the child).
        ...
        {
		int val;
                // pid2 = waitpid(pid,&val,0);
                pid2 = wait(&val);
                if(WIFEXITED(val)){
                        Total = Total + WEXITSTATUS(val);
                }

		//call function B to perform the summation from [(x/2)+1] to x.
                Total = Total + B(x);
                // printf("pid = %d, pid2 = %d,val = %d\n",pid,pid2,WEXITSTATUS(val));
        }
        else 
        {
